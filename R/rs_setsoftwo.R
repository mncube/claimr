...
#' includes the random numbers (in the order variable) and each has been sorted by first_set and second_set.  The input list
...
rs_setsoftwo <- function(df = NULL,
  ...
  frame_length <- (first_high - first_low + 1)*(second_high - second_low + 1)

  #Add values and random numbers ("order") to a data frame
  frame <- expand.grid(first_value = first_low:first_high, second_value = second_low:second_high)
  ...
  if (quantity == 0){
    ...
    df_sample <- df_sample[with(df_sample, order(first_value, second_value)), ]
  ...
  if (spares == 0 | quantity == 0){
    ...
    #order data frames by values then reset names
    df_spare <- df_spare[with(df_spare, order(first_value, second_value)), ]
  ...
  #order data frames by values then reset names
  df <- df[with(df, order(first_value, second_value)), ]
